...
export const vehicleType = defineType({
  ...
  fields: [
    ...
    defineField({ name: 'images', title: 'Advert photos', type: 'array', group: 'photos', description: 'First photo becomes the main advert photo. Drag photos to reorder.', of: [{ type: 'image', options: { hotspot: true } }] }),
    defineField({ name: 'imageUrls', title: 'Current website photo links', type: 'array', group: 'photos', description: 'Used for imported current adverts. For new adverts, upload photos above instead.', of: [{ type: 'url' }], readOnly: true }),
